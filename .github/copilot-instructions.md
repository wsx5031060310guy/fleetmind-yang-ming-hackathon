...
Architecture and pipeline decisions follow `docs/09-architecture-and-execution-plan.md` §2–§4 and `docs/12-requirements-fit-and-final-architecture.md` §4: core-calc Java pure library, single Spring Boot service (same-origin React dashboard), full-dataset Daily FOC calculation with quality flags (no row dropping). Do not re-propose services eliminated in `docs/11` (ECS, RDS, CloudFront, QuickSight, Bedrock Agents).

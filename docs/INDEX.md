...
| 10 presentation-plan | 8 分鐘 slide 計畫 | 🔄 待依 13 回寫 |
...
- **P5 成員（簡報/提交）**：`02` 分工 → `10` 全文 → `13` §3.2 呈現項+§4 → `12` §3（G1/G4/G5 提交缺口）→ `09` §7 只看 P5 欄 → `14` PR-6/PR-9 → 本頁 §6 checklist。
...
- 全部規劃在 **PR #1** 待 team review；待拍板事項見 `14` §4。
- 待回寫（review 後執行）：`10` 依 `13` 重排、`06` 時間軸、`09` §4.5 升級項——追蹤 `14` §2。

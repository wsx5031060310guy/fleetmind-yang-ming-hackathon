...
| R3 | Day3 提交**七項** checklist（Sunny owner，逐項對官方清單） | ⚠️ G1/G4/G5 | 「live demo 連結」「企業資料應用說明」「challenge link」三個缺口見 §3 |
...
R3 是硬性需求，此項漏交＝提交不完整，不是「建議」等級。素材直接取自 `09` §3–§4、不依賴 Day3 凍結資料，**沒有理由押到最後一刻**：Day2 晚間（與 slides 主體同時段）由 Feng 產出 1–2 頁初稿（資料欄位使用方式、篩選條件實作、VLSFO 換算、Daily FOC、Speed Loss 方法摘要），Day3 上午只做定稿校對。

### G5【必修】Challenge link 無人承載

官方七項提交物中的 **challenge link** 在 `09` §7「五件套」與本文件初版均無承載元件——漏交任一必交項在 R4「逾時即棄權」的規則下直接出局。修正：

- `09` §7 Day3 的「五件套」全部改稱「**官方七項提交物**」，Sunny 的核對清單逐項列出：deck／challenge link／企業資料應用說明／技術架構／GitHub repo／live demo 連結／錄影。
...
| 時點 | 增量 | Owner |
| --- | --- | --- |
| Day1 09:40–10:00 | 上傳平台公布：記錄欄位／檔案格式限制、challenge link 定義、demo 連結欄位格式（G1/G5/R20） | 全員記錄 |
| Day1 10:00–10:40 | 待確認清單追加：live demo 連結雲端全掛時可否以錄影替代（G1）；資料保留／刪除規定（G3）；水下報告是否為照片型、事件欄位如何取得（R5） | 全員記錄 |
| Day1 16:00 | 抽籤結果記入 Day3 彩排排程（R22） | Eddie |
| Day2 | ai-brief prompt 含事件關聯段，與 before-after API 同步交付 | Eddie |
| Day2 晚間 | 「企業資料應用說明」1–2 頁初稿（素材自 `09` §3–§4，與 slides 主體同時段） | Feng |
| Day3 07:30–11:00 | 企業資料應用說明定稿校對 | Feng |
| Day3 12:00–14:00 | 上傳前 warm-up 腳本跑一輪；**官方七項提交物逐項核對**（含 repo 無原始資料／憑證檢查 R19）；清理 runbook 確認 | Sunny |
| 賽後 | 若 Day1 確認需刪除：執行資料清理 runbook（raw/、processed/、exports/、本機快照） | Sunny |

## 6. 結論

- 題目 22 項需求：18 項 ✅ 原生符合；4 項需求（R3／R12／R16／R20）存在共 5 個缺口（G1–G5，其中 R3 對應 G1+G4+G5），全部以「改流程／改 prompt／加交付物」修正，**零架構變更**。
